...
class G1_29_ArmController:
    def __init__(self, motion_mode = False, simulation_mode = False):
        ...
        self.q_target = None  # 将在 DDS 订阅成功后设为当前值
        ...
        # ★ 关键：初始 q_target 设为当前手臂真实角度，避免接管瞬间跳到零位
    ...
    def _ctrl_motor_state(self):
        # ★ motion mode 权重平滑渐入：从 0 → 1 ，约 2 秒 (500 步 @250Hz)
        if self.motion_mode:
            ramp_steps = 500
            for i in range(ramp_steps):
                ...
                if not self.simulation_mode:
                    arm_q_target = self.clip_arm_q_target(arm_q_target, velocity_limit=self.arm_velocity_limit)
                for idx, mid in enumerate(G1_29_JointArmIndex):
                    self.msg.motor_cmd[mid].q   = arm_q_target[idx]
            ...
            logger_mp.info("[G1_29_ArmController] Motion mode weight ramp-up complete (0→1).")
    ...
    def smooth_exit(self):
        '''平滑退出 motion mode：将权重从 1 渐变到 0，约 2 秒。'''
        if not self.motion_mode:
            return
        logger_mp.info("[G1_29_ArmController] Motion mode weight ramp-down (1→0)...")
        ramp_steps = 500  # ~2秒 @250Hz
        for i in range(ramp_steps):
            w = 1.0 - (i + 1) / ramp_steps
            self.msg.motor_cmd[G1_29_JointIndex.kNotUsedJoint0].q = w
            self.msg.crc = self.crc.Crc(self.msg)
            self.lowcmd_publisher.Write(self.msg)
            time.sleep(self.control_dt)
        logger_mp.info("[G1_29_ArmController] Motion mode weight ramp-down complete.")
