...
class WebRTC_PublisherThread(threading.Thread):
    ...
    def __init__(self, port: int, host: str = "0.0.0.0", codec_pref: str = None):
        ...
        if AIORTC_IMPORT_ERROR is not None:
            raise RuntimeError(
                "WebRTC dependencies are missing. Install teleimager with "
                '`pip install -e ".[server]"` or disable enable_webrtc in cam_config_server.yaml.'
...
def reload_uvc_driver():
    try:
        subprocess.run(["sudo", "-n", "modprobe", "-r", "uvcvideo"], check=True)
        time.sleep(1)
        subprocess.run(["sudo", "-n", "modprobe", "uvcvideo", "debug=0"], check=True)
        time.sleep(1)
        logger_mp.info("UVC driver reloaded successfully.")
    except subprocess.CalledProcessError as e:
        logger_mp.warning(f"Skipping UVC driver reload: {e}")
